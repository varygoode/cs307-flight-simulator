...
//----------------------------------------
void CityDataParser::InitCityData(char *dataFile)
{
	...
	while(getNextLine(line, 127))
	{
		...
		else if(strcmp(line, "<PIXWIDTH>") == 0)
		{
			getNextLine(line, 127); // Read city symbol
			m_iMapImageWidth = atoi(line);
		}
		else if(strcmp(line, "<PIXHEIGHT>") == 0)
		{
			getNextLine(line, 127); // Read city symbol
			m_iMapImageHeight = atoi(line);
		}
	}
	inFile->close();
	delete inFile;
...
//--------------------------------------------------------------
bool CityDataParser::getCityData(char *name, char *state, char *symbol, 
		...
		if(strcmp(line, "<CITY>") == 0)
		{
			if(thisCount == nextCityIdx) // Want this one
			{
				while(strcmp(line, "</CITY>") != 0)
				{
					...
					else if(strcmp(line, "<PIXLOCX>") == 0)
					{
						getNextLine(line, 127); // Read city symbol
						*xPos = atoi(line);
					}
					else if(strcmp(line, "<PIXLOCY>") == 0)
					{
						getNextLine(line, 127); // Read city symbol
						*yPos = atoi(line);
					}
...
//------------------------------------------------
// Get the size in pixels of the map image
//------------------------------------------------
void CityDataParser::getMapImageSize(int *width, int *height)
{
	*width = m_iMapImageWidth;
	*height = m_iMapImageHeight;
}
